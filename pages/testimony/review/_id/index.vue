          ...
          <v-card-title class="d-flex flex-column align-start">
            <div class="d-flex align-center justify-space-between" style="width: 100%">
              ...
              <div class="text-right">
                ...
                <v-chip v-else-if="mTestimony.status === 'rejected'" color="red" text-color="white"
                  small>RECHAZADO</v-chip>
                <v-chip v-else color="grey" small>Pendiente</v-chip>
                <div v-if="mTestimony.status_username" class="grey--text text-caption mt-1">Por: {{
                  mTestimony.status_username }}</div>
              </div>
            </div>
            <div class="mt-2">
              <v-chip v-for="(c, i) in mTestimony.categories || []" :key="i" class="ma-1" color="primary" small>{{ c
                }}</v-chip>
            </div>
          </v-card-title>
              ...
              <v-col cols="12" md="4">
                ...
                <div v-if="mTestimony.link" class="mt-3">
                  <v-responsive v-if="embedSrc" aspect-ratio="16/9">
                    <iframe :src="embedSrc" frameborder="0" style="width: 100%; height: 100%"
